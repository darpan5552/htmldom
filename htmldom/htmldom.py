...
class HtmlDomNode:
    ...
    def remove(self, node):
        ...
        if parent_node:
            try:
                ...
                raise Exception(
                    str(node) + ": node is not a children of the parent node")
    ...
    def append(self, node):
        if isinstance(node, HtmlDomNode):
            ...
            raise Exception(
                "Invalid node object. object must be of type HtmlDomNode.")
    ...
    def prepend(self, node):
        if isinstance(node, HtmlDomNode):
            ...
            raise Exception(
                "Invalid node object. object must be of type Element.")
        ...
        if isinstance(target, HtmlDomNode) and isinstance(src, HtmlDomNode):
            if not flag:
                ...
                try:
                    ...
                    raise Exception(
                        "source node object must be children of the parent object")
...
#               self.domNodes[ nodeName ] += self.getUniqueNodes( self.domNodes[ nodeName ], [ domNode ] )
        ...
        for value in selectors:
            ...
            if match:
                ...
                while data and invalid != 100:
                    ...
                    if match:
                        ...
                        # attribute selector
                            ...
                            attrMatch = re.search(
                                attributeSubStringSelector, attr)
                            if attrMatch:
                                attributeSelectorFlags[attrMatch.group(
                                    1)] = True
                                _index = attr.find(attrMatch.group(1))
                                attr = attr[:_index -
                                            len(attr)] + attr[_index + 1:]
                                attr = attr.split("=")
                            elif attr.find("=") == -1:
                            ...
                            if attr[1] == '':
                                attributeSelectorFlags["noVal"] = True
                            attr_list.append(
                                (dict(attributeSelector), dict(attributeSelectorFlags)))
                            attributeSelector = {}
                            attributeSelectorFlags = {
                                '$': False, '^': False, '*': False, 'noVal': False, "~": False
                            }
                ...
                if elemName:
                    ...
                    if classSelector:
                        nodes = self.getNodesWithClassOrId(
                            classSelector[-1], selectType='class')
                    elif idSelector:
                        nodes = self.getNodesWithClassOrId(
                            idSelector[-1], selectType='id')
                    elif attr_list:
                        nodes = []
                ...
                for node in nodes:
                    ...
                    if nodeAccepted:
                        for a_s, a_f in attr_list:
                            nodeAccepted = self.getNodesWithAttributes(
                                a_s, a_f, [node])
                            if not nodeAccepted:
                                break
                    if nodeAccepted:
    ...
    def getNodesWithClassOrId(self, className="", nodeList=None, selectType=""):
        tmpList = []
        for nodeName in self.domNodes:
            [tmpList.append(selectedNode) for selectedNode in self.domNodes[nodeName]
             if className in selectedNode.attributes.get(selectType, [''])]
        return tmpList
...
class HtmlNodeList:
        ...
        if not all_children:
            for node in self.nodeList:
                [childrenList.append(child)
                 for child in node.children if child.nodeType == 1]
        else:
            for node in self.nodeList:
        ...
        for node in self.nodeList:
            ...
            for selectedNode in nList.nodeList:
                try:
                    ...
                    siblingsSet += self.getUniqueNodes(
                        siblingsSet, selectedNodeList)
                    break
                except ValueError:
                    pass
            else:
        ...
        for node in self.nodeList:
            ...
            for selectedNode in nList.nodeList:
                try:
                    index = tmpList.index(selectedNode)
                    selectedNodeList = tmpList[:index]
                    parentsList += self.getUniqueNodes(
                        parentsList, selectedNodeList)
                    break
                except ValueError:
                    pass
            else:
    ...
    def add(self, selector):
        ...
        """
        nList = self.htmlDom.find(selector)
        newNodeList = self.nodeList + \
            self.getUniqueNodes(self.nodeList, nList.nodeList)
    ...
    def contains(self, pattern):
        ...
        for node in self.nodeList:
            ...
            if pattern.search(text):
                selectedNodeList += self.getUniqueNodes(
    ...
    def appendTo(self, nodes, context=None):
        ...
        if isinstance(nodes, HtmlNodeList):
            ...
            if context and isinstance(context, HtmlDom):
                context.find(nodes).append(self)
            else:
